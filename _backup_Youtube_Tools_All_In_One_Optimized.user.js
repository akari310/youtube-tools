...
<svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-circle-x" width="24"
  ...
  const ytmAmbientMode = {
    ...
    _getArtUrl() {
      ...
      for (const sel of selectors) {
        ...
        if (img && img.src && img.src.startsWith('http')) {
          return img.src.replace(/=w\d+-h\d+/, '=w640-h640')
            .replace(/=s\d+/, '=s640');
        }
      }
      const video = $e('video');
    ...
    _ensureInit() {
      ...
      this.styleEl.textContent = `
        #ytm-ambient-glow {
          ...
          background-position: center;
          background-repeat: no-repeat;
          filter: blur(80px) saturate(1.4) brightness(0.7);
          transition: opacity 0.6s ease;
        }
        #ytm-ambient-glow.active {
          opacity: 0.4;
        }
        /* Make only player page backgrounds transparent so glow shows through */
        body:has(#ytm-ambient-glow.active) ytmusic-app,
        body:has(#ytm-ambient-glow.active) #layout {
          background-color: transparent !important;
          background: transparent !important;
          transition: background-color 0.6s ease;
        }
        body:has(#ytm-ambient-glow.active) ytmusic-player-page,
        body:has(#ytm-ambient-glow.active) #player-page {
          background-color: transparent !important;
          background: transparent !important;
        }
        /* Keep home/browse pages opaque so they don't bleed through */
        body:has(#ytm-ambient-glow.active) ytmusic-browse-response {
          background-color: #0d0d0d !important;
        }
        body:has(#ytm-ambient-glow.active) {
          background-color: #0d0d0d !important;
        }
      `;
    ...
    show() {
      ...
      this.videoEl = $e('video');

      // Update art and show glow
      this._updateArt();
  ...
  function applySettings() {
    ...
    function applyYTMThemeVars(bgColor, textColor, secondaryText, menuBg, iconColor, raisedBg) {
  ...
  function buildYTMToolbar() {
    ...
    [['', 'Calidad del video / Quality video', true],
    ...
    [['', 'Calidad del Audio / Quality Audio', true],
  ...
  function renderizarButtons() {
    if (isYTMusic) {
      ...
      if (addButton && validoBotones) {
        validoBotones = false;
        // YTM has strict Trusted Types: use pure DOM API
        const toolbar = buildYTMToolbar();
        addButton.parentElement.insertBefore(toolbar, addButton);
      }
    } else {
      // Regular YouTube
  ...
  document.addEventListener('yt-navigate-finish', () => {
    ...
    if (isYTMusic) {
      ...
      if (document.location.href.includes('watch')) {
        ...
        const savedSettings = JSON.parse(GM_getValue('ytSettingsMDCM', '{}'));
