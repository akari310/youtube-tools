...
export function updateContinueWatchingButton() {
  const rt = __ytToolsRuntime.continueWatching;
...
export function navigateToWatchSpa(videoId, seconds) {
...
// ------------------------------
// Feature: Continue Watching UI (ported from legacy)
// ------------------------------

export function updateContinueWatchingHistoryUi() {
  const rt = __ytToolsRuntime.continueWatching;
...
export function updateContinueWatchingPanelRow(videoId) {
  try {
    const rt = __ytToolsRuntime.continueWatching;
...
export function renderContinueWatchingPanel() {
  const panel = $id('yt-continue-watching-panel');
  if (!panel) return;
...
export function setupContinueWatchingFeature(enabled) {
  const rt = __ytToolsRuntime.continueWatching;
  rt.enabled = !!enabled;
  ...
  // Pagehide handler - save on tab close
    ...
    window.addEventListener(
      ...
      () => {
        try {
          ...
          if (rt.map)
            writeJsonGM(STORAGE_KEYS_MDCM.CONTINUE_WATCHING, pruneContinueWatchingMap(rt.map, 200));
        } catch (e) {
  ...
  // Not on watch page or disabled: detach video listeners and hide
    ...
    rt.handlers = {
      timeupdate: () => {
        try {
          ...
          if (!vid) return;
          const now = Date.now();
          if (now - rt.lastSaveAt < 5000) return;
          if (v.paused) return;
          const t = Number(v.currentTime);
          const d = Number(v.duration);
