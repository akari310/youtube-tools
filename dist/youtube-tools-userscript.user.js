...
(function() {
	...
	function isStatsPanelVisible() {
		if (!$id("stats")?.classList?.contains("active")) return false;
	...
	var DEFAULT_SETTINGS = {
		...
		theme: "0",
		themeCustom: false,
		bgColorPicker: "#0b1016",
		primaryColorPicker: "#ffffff",
		secondaryColorPicker: "#a7b4c2",
		headerColorPicker: "#0f1721",
		iconsColorPicker: "#22d3ee",
		menuColorPicker: "#0f1721",
		lineColorPicker: "#22d3ee",
		timeColorPicker: "#ffffff",
		progressbarColorPicker: "#22d3ee",
		backgroundImage: "",
		ambientMode: false,
		...
		waveVisualizerFps: "30",
		waveVisualizerIntensity: "100",
		ytmWaveColor: "#22d3ee",
		ytmWaveHeight: "36",
		ytmWavePlacement: "edge",
		ytmPlayerBarOpacity: "72",
		ytmPanelOpacity: "66",
		ytmPanelBlur: "22",
  ...
  `);
	...
	var WAVE_MIN_HEIGHT = 58;
	var WAVE_YTM_HEIGHT = 36;
	var WAVE_YTM_CANVAS_Z_INDEX = 130;
	var WAVE_SENSITIVITY = 2.05;
	var WAVE_YTM_SENSITIVITY = 5.5;
	var videoObserver = null;
	var observerDebounce = null;
	var frequencyData = null;
	var lastYtmFrameAt = 0;
	...
	function getWaveIntensityScale() {
		...
		return Math.max(.4, Math.min(1.6, pct / 100));
	}
	...
	function getYtmWaveTopOffset(playerBarRect) {
		...
		switch (waveSettingsSnapshot?.ytmWavePlacement) {
			...
			case "bottom": return Math.max(0, Math.floor((playerBarRect?.height || 90) - waveHeight - 4));
			default: return Math.min(-8, Math.round(-waveHeight * .66));
		}
	}
	...
	function updateCanvasSize() {
		if (s.canvas) {
			...
			const playerBarRect = (isYTMusic$1 ? getYtmPlayerBar() : null)?.getBoundingClientRect?.();
			const width = Math.max(1, Math.floor(playerBarRect?.width || PW().innerWidth));
			const height = isYTMusic$1 ? getYtmWaveHeight() : Math.max(WAVE_MIN_HEIGHT, Math.min(240 * .34, Math.floor(PW().innerHeight * .09)));
			if (isYTMusic$1 && playerBarRect) {
	...
	function getYtmPlayerBar() {
		const playerBar = PD().querySelector("ytmusic-player-bar") || PD().querySelector("#player-bar") || PD().querySelector("ytmusic-app #player");
		if (playerBar?.style?.position === "relative") playerBar.style.removeProperty("position");
		return playerBar;
	}
	...
	function showCanvas() {
		...
		if (canvas) {
			updateCanvasSize();
			canvas.style.opacity = isYTMusic$1 ? "0.86" : "0.82";
		}
	...
	function createVisualizerOverlay() {
		...
		if (isYTMusic$1) getYtmPlayerBar();
		const canvasStyle = isYTMusic$1 ? `position:fixed;top:0;left:0;width:100%;height:${WAVE_YTM_HEIGHT}px;pointer-events:none;z-index:${WAVE_YTM_CANVAS_Z_INDEX};opacity:0;background:transparent;transition:opacity 0.35s ease;` : "position:fixed;bottom:0;left:0;width:100%;pointer-events:none;z-index:1;opacity:0;background:transparent;transition:opacity 0.35s ease;";
		if (existing) {
			existing.style.cssText = canvasStyle;
			if (existing.parentNode !== PD().body) PD().body.appendChild(existing);
			setCanvas(existing);
	...
	function drawAmbientFloor(w, h, colors) {
		...
		floor.addColorStop(.62, colors.soft);
	...
	function waveY(index, centerY, amplitudeRange) {
		return centerY + Math.max(-1, Math.min(1, (s.smoothedData[index] - 128) / 128 * WAVE_SENSITIVITY * getWaveIntensityScale())) * amplitudeRange;
	}
	function drawYtmPlayerBarWave(w, h, colors) {
		...
		if (frequencyData?.length) {
			...
			for (let i = start; i < end; i++) energy += frequencyData[i] / 255;
			energy /= Math.max(1, end - start);
		}
		const centerY = h * .68;
		const amplitudeRange = h * (.1 + Math.min(.18, energy * .55));
		const gradient = s.ctx.createLinearGradient(0, 0, w, 0);
		gradient.addColorStop(0, colors.accent + "11");
		gradient.addColorStop(.35, colors.accent + "aa");
		gradient.addColorStop(.5, "#ffffffcc");
		gradient.addColorStop(.65, colors.accent + "aa");
		gradient.addColorStop(1, colors.accent + "11");
		const glow = s.ctx.createLinearGradient(0, 0, 0, h);
		glow.addColorStop(0, "rgba(0,0,0,0)");
		glow.addColorStop(.58, colors.soft);
		...
		for (let i = 0; i < s.bufferLength; i++) {
			const y = centerY + Math.max(-1, Math.min(1, (s.smoothedData[i] - 128) / 128 * WAVE_YTM_SENSITIVITY * getWaveIntensityScale())) * amplitudeRange;
			points.push([x, y]);
	...
	function draw(frameTime = 0) {
		...
		if (isYTMusic$1) {
			...
			const frameInterval = 1e3 / getWaveFps();
		...
		const centerY = isYTMusic$1 ? h * .58 : h * .34;
		const amplitudeRange = isYTMusic$1 ? h * .5 : h * .42;
		if (isYTMusic$1) {
		...
		switch (style) {
			...
			case "barras":
				...
				x = 0;
				{
					...
					for (let i = 0; i < bars; i++) {
						...
						const value = Math.min(1, (frequencyData[bucket] || 0) / 255 * WAVE_SENSITIVITY * getWaveIntensityScale());
			...
			case "dinamica": {
				...
				gradient.addColorStop(.35, accent + "dd");
				gradient.addColorStop(.65, "#ffffff99");
				...
				if (!isYTMusic$1) {
					...
					fill.addColorStop(.55, accent + "06");
  ...
  `;
	...
	var ambientWatcherId = null;
	var ambientWatcherStartTimeout = null;
	var ambientPageDataHandler = null;
	var ambientNavResetBound = false;
	var ytmAmbientMode = {
		active: false,
		_initialized: false,
		glowEl: null,
	...
	function startAmbientWatcher() {
		...
		function start() {
			...
			ambientWatcherId = trackInterval(setInterval(() => {
				if (document.visibilityState !== "visible") return;
		...
		if (!ambientWatcherStartTimeout && !ambientWatcherId) ambientWatcherStartTimeout = trackTimeout(setTimeout(() => {
			ambientWatcherStartTimeout = null;
			start();
		}, 3e3));
		if (!ambientPageDataHandler) ambientPageDataHandler = trackEventListener(document, "yt-page-data-updated", () => {
...
`;
	...
	var selectedBgColor = "#0f1721";
	var selectedTextColor = "#eef6fb";
	var selectedBgAccentColor = "#22d3ee";
	...
	function applySettings() {
		...
		$sp("--yt-tools-menu-font-size", `${Math.max(11, Math.min(16, Number(settings.menuFontSize) || 13))}px`);
		$sp("--yt-tools-ytm-player-alpha", `${clampNumber(settings.ytmPlayerBarOpacity, 35, 95, 72) / 100}`);
		...
		if (settings.ytmWaveColor) waveColor = settings.ytmWaveColor;
		else if (settings.themes && isDarkMode === "dark") {
			if (isThemeCustom) waveColor = settings.progressbarColorPicker || "#22d3ee";
			else if (selectedTheme) waveColor = selectedTheme.waveColor || "#22d3ee";
		}
    ...
    `);
      ...
      ytmusic-player-bar {
        background: linear-gradient(180deg, rgba(15, 23, 33, ${Math.max(.35, ytmPlayerAlpha - .14)}), rgba(7, 11, 18, ${ytmPlayerAlpha})) !important;
	...
	function loadSettingsToDOM() {
		...
		if (waveIntensityLabel) waveIntensityLabel.textContent = $id("wave-intensity-slider")?.value || "100";
		...
		if (playerOpacityLabel) playerOpacityLabel.textContent = $id("ytm-player-opacity-slider")?.value || "72";
		const panelOpacityLabel = $id("ytm-panel-opacity-value");
		if (panelOpacityLabel) panelOpacityLabel.textContent = $id("ytm-panel-opacity-slider")?.value || "66";
		...
		if (menuFontSizeLabel) menuFontSizeLabel.textContent = $id("menu-font-size-slider")?.value || "13";
	...
	function setupSettingsPanelEvents(panelDOM) {
		...
		if (waveIntensitySlider && waveIntensityValue) waveIntensitySlider.addEventListener("input", () => {
		...
		if (ytmWaveColorPicker) ytmWaveColorPicker.addEventListener("input", persistApplyAndNotifyFeatures);
		...
		if (ytmWaveHeightSlider && ytmWaveHeightValue) ytmWaveHeightSlider.addEventListener("input", () => {
		...
		if (ytmPlayerOpacitySlider && ytmPlayerOpacityValue) ytmPlayerOpacitySlider.addEventListener("input", () => {
		...
		if (ytmPanelOpacitySlider && ytmPanelOpacityValue) ytmPanelOpacitySlider.addEventListener("input", () => {
		...
		if (ytmPanelBlurSlider && ytmPanelBlurValue) ytmPanelBlurSlider.addEventListener("input", () => {
		...
		if (menuFontSizeSlider && menuFontSizeValue) menuFontSizeSlider.addEventListener("input", () => {
